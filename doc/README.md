...
### 进行部署
kube-chaos以Daemonset的方式部署，部署配置在项目目录中的`chaos-daemonset.yaml`中，在kube-chaos镜像生成后，使用kubectl根据该配置文件来部署kube-chaos:
`kubectl apply -f chaos-daemonset.yaml`

项目中有一个`autodeploy.sh`文件，它包含了该条指令，执行`sh autodeploy.sh`效果相同。
...
```
kubectl annotate pod $1 kubernetes.io/ingress-chaos="{\"Delay\":{\"Set\":\"yes\",\"Time\":\"200ms\",\"Variation\":\"50ms\"}}" kubernetes.io/done-ingress-chaos=no --overwrite
```

并且要注意的是，要设置`kubernetes.io/done-ingress-chaos=no`或`kubernetes.io/done-egress-chaos=no`以使设置生效。
